...
A A x
  ...
  lazy val threeXes: Tree = new Tree(
    Token("S"),
    Seq(
      new Tree(Token("BOF")),
      new Tree(
...
A A x A
  ...
  /** A **different** parse tree of the **same** string as `tree1` according to
    * `ambiguousGrammar`.
    */
  lazy val tree2: Tree = new Tree(
    Token("S"),
    Seq(
      new Tree(Token("BOF")),
      new Tree(
